...
const GUI = (cvs, glWindow, place) => {
	...
	let touchstartTime;
let lastClickTime = 0;
const CLICK_DELAY_THRESHOLD = 100; // Adjust the threshold as needed, in milliseconds.
...
cvs.addEventListener("mousedown", (ev) => {
  // Calculate the time difference between the current click and the last click.
  const currentTime = Date.now();
  const timeSinceLastClick = currentTime - lastClickTime;

  // Check if the time difference is greater than the threshold.
  if (timeSinceLastClick >= CLICK_DELAY_THRESHOLD) {
    switch (ev.button) {
      case 0:
        dragdown = true;
